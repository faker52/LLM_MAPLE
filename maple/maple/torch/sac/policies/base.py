...
class TorchStochasticPolicy(
    ...
    def get_action(self, obs_np, pop_id=None, return_dist=False):
        info = {}
        if return_dist:
            actions, dist = self.get_actions(obs_np[None], return_dist=return_dist)
            info['dist'] = dist
        else:
            actions = self.get_actions(obs_np[None], pop_id, return_dist=return_dist)
        return actions[0, :], info

    def get_actions(self, obs_np, pop_id=None, return_dist=False):
        dist = self._get_dist_from_np(obs_np)
        if pop_id is None:
            actions = dist.sample()
        else:
            actions = dist.sample(pop_id)
        if return_dist:
